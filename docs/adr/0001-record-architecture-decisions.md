...
Quartermaster makes a number of decisions that are significant and not easily
reversed: the relational/transactional core, the two-flavour optimistic
concurrency strategy, the idempotency and isolation model, and the
inbound/outbound decoupling. Without a durable record of why each choice was
made, the reasoning is lost, and later contributors are left to reconstruct it
or unknowingly undo it.
